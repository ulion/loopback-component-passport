 ...
 */
 module.exports = function(UserIdentity) {
  ...
  UserIdentity.login = function (provider, authScheme, profile, credentials,
    ...
    console.log(['UserIdentity.login', provider, authScheme, profile, credentials, options]);
    ...
    userIdentityModel.findOne({where: {
      ...
      console.log(['UserIdentity.login findOoe', err, identity]);
      if (err) {
        return cb(err);
      }
      if (identity) {
        ...
        return identity.updateAttributes({profile: profile,
          ...
          console.log(['UserIdentity.login updateAttributes', err, i]);
          ...
          return identity.user(function (err, user) {
            ...
            console.log(['UserIdentity.login user', err, user]);
      ...
      console.log(['UserIdentity.login userObj', userObj]);
      if (!userObj.email) {
      ...
      userModel.findOrCreate({where: {or: [
        ...
        console.log(['UserIdentity.login findOrCreate', err, user]);
        if (err) {
          return cb(err);
        }
        var date = new Date();
        userIdentityModel.create({
          ...
          console.log(['UserIdentity.login create', err, identity]);
          if(!err && user && autoLogin) {
            return (options.createAccessToken || createAccessToken)(user, function(err, token) {
              cb(err, user, identity, token);
            });
